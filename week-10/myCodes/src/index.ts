...
async function insertData() {
  const client = new Client({
    connectionString:
      "postgresql://neondb_owner:G01sYKodFIuT@ep-lively-leaf-a54lecny-pooler.us-east-2.aws.neon.tech/neondb?sslmode=require",
  });
...
async function insertSqlInjectionFreeData(
  username: string,
  email: string,
  password: string
) {
  ...
  try {
    ...
    console.log("Insertion success without sql injection:\n", res);
  } catch (err) {
    console.error("Error during the insertion:", err);
  } finally {
    await client.end();
